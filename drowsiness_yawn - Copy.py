...
while True:
    ...
    frame = vs.read()
    frame_1 = imutils.resize(frame, width=450)
    frame = frame_1.copy()
    frame_12 = frame_1.copy()
    ...
    if len(faces) > 0:
                ...
                for start, end in line_pairs:
                    cv2.line(frame_1, (int(reprojectdst[start][0]), int(reprojectdst[start][1])),
                             (int(reprojectdst[end][0]), int(reprojectdst[end][1])),
                             (0, 0, 255))
##                    cv2.line(frame_1, reprojectdst[start], reprojectdst[end], (0, 0, 255))
   ...
   # frame = vs.read()
    #frame = imutils.resize(frame_12, width=450)
    gray = cv2.cvtColor(frame_12, cv2.COLOR_BGR2GRAY)
    ...
    for (x, y, w, h) in rects:
        ...
        lip = shape[48:60]
        cv2.drawContours(frame_12, [lip], -1, (0, 255, 0), 1)
        ...
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                    ...
                    cv2.putText(frame_12, "DROWSINESS ALERT!", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
##                    pygame.init()
##                    pygame.mixer.init()
##                    pygame.mixer.music.load("drow.mp3")
##                    pygame.mixer.music.play()
        ...
        if (distance > YAWN_THRESH):
                print('Yawn  ALERT!')
                cv2.putText(frame_12, "Yawn Alert", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
##                pygame.init()
##                pygame.mixer.init()
...
##                pygame.mixer.music.play()
            ...
            cv2.putText(frame_12, "EAR: {:.2f}".format(ear), (300, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.putText(frame_12, "YAWN: {:.2f}".format(distance), (300, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
##            pygame.init()
##            pygame.mixer.init()
##            pygame.mixer.music.load("yawn.mp3")
